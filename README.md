...
The Agile technique prioritises iterative development and collaboration, I have chosen this approach to plan and carry out this project. The project starts by outlining its overall objective, which is to provide a user-friendly ticket submission system for the User Requirement Specifications (URS). A product backlog will be organised into epics like "UI Design," "Form Functionality," and "Data Storage," containing the requirements, each of which will be divided into smaller, more manageable tasks. The project will be broken up into three two-week-long sprints, each with their own set of deliverable goals. Daily standups will then be used to track progress and resolve any obstacles with the help of regular sprint planning sessions which can be organised to prioritise work. A review will be conducted at the conclusion of each sprint to review the results and take feedback into consideration. 

In order to improve the process for the sprints, re-evaluations will also be carried out. In order to maintain team transparency and visualise the workflow, we can make use of a Kanban board like Sharepoint Dashboard. Throughout the development cycle, testing and debugging are essential in order to maintain the quality of the final product. This strategy makes sure that the project will be completed in a flexible and progressive manner while successfully fulfilling the requirements.
